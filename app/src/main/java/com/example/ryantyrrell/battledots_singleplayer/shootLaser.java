...
public class shootLaser {
    ...
    public shootLaser(Canvas canvasParameter, ManageObjects manageObjects)     {
    ...
    public void updateAndDrawLaser(int list_num, double dot_location_x, double dot_location_y, double instantaneous_movement_x, double instantaneous_movement_y) {
        ...
        if (laser_collide_with_2 == false)
            checkLaserLocation(list_num);


    }

    public void checkLaserLocation(int list_num)     {
        ...
        if (laser_y_coordinates < top || laser_y_coordinates > bottom
            ...
            MANAGE_OBJECTS_OBJECT.removeLaserFromList(list_num);   //Gives the go ahead for laser to be remove from the list
    ...
    public boolean checkLaserCollision(int list_num, double dot_location_x, double dot_location_y) {
        ...
        if (MANAGE_OBJECTS_OBJECT != null) {
            ...
            if (distance <= LASER_RADIUS + dot_radius) {
                MANAGE_OBJECTS_OBJECT.removeLaserFromList(list_num);  //Gives the go ahead for laser to tbe remove from the list
                return true;
            }
        }
        return false;
